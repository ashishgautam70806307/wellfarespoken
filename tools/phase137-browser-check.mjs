...
const phpPort = Number(process.env.PHASE137_PHP_PORT || 18775);
const chromePort = Number(process.env.PHASE137_CHROME_PORT || 19232);
...
async function runBrowserChecks() {
  tempDir = await mkdtemp(path.join(tmpdir(), 'phase137-chrome-'));
  const prepend = path.join(root, 'tools', 'phase137-fixture-bootstrap.php');
  ...
  check('Footer: logo, dynamic social icons and useful links render', footer.logoVisible && footer.socials >= 4 && footer.phone !== '' && footer.links >= 12, JSON.stringify(footer));
  ...
  check('Admission: universal fields are smooth and non-overlapping', admissionFields.length >= 6 && admissionFields.every(item => item.radius >= 12 && item.height >= 50 && item.width > 150 && item.left >= 0 && item.right <= item.viewport + 1), JSON.stringify(admissionFields));
  ...
  check('Student Register: balanced two-panel form and universal inputs', register.fields >= 7 && register.copy > 280 && register.card > register.copy && register.radius >= 12 && register.height >= 50 && register.overflow <= 3, JSON.stringify(register));
...
const output = path.join(root, 'PHASE137_BROWSER_VALIDATION.json');
