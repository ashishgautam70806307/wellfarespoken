...
function p139_check(string $name, string $status, string $detail = ''): void
{
...
function p139_scalar(string $sql, array $params = []): mixed
{
...
p139_check('Runtime environment', 'PASS', APP_RUNTIME_ENV . ' / ' . APP_ENV);
p139_check('Database profile', 'PASS', DB_HOST . ':' . DB_PORT . '/' . DB_NAME . ' as ' . DB_USER);
p139_check('Runtime schema updates', APP_ALLOW_SCHEMA_UPDATES ? 'WARN' : 'PASS', APP_ALLOW_SCHEMA_UPDATES ? 'Enabled; disable after import.' : 'Disabled');
p139_check('Application debug', (APP_RUNTIME_ENV === 'live' && APP_DEBUG) ? 'FAIL' : 'PASS', APP_DEBUG ? 'Enabled' : 'Disabled');
p139_check('UTF-8 compatibility', function_exists('mb_strlen') && function_exists('mb_substr') && function_exists('mb_strimwidth') ? 'PASS' : 'FAIL', extension_loaded('mbstring') ? 'mbstring' : 'compatibility fallback');

$mysqlDriver = in_array('mysql', PDO::getAvailableDrivers(), true);
p139_check('PDO MySQL driver', $mysqlDriver ? 'PASS' : 'FAIL', implode(', ', PDO::getAvailableDrivers()) ?: 'No PDO drivers');
$connected = false;
if ($mysqlDriver) {
    try {
        db();
        $connected = true;
        p139_check('Database connection', 'PASS');
    } catch (Throwable $exception) {
        p139_check('Database connection', 'FAIL', 'Connection failed. Verify local/live .env settings.');
    }
} else {
    p139_check('Database connection', 'FAIL', 'pdo_mysql is not installed in this PHP runtime.');
}
...
if ($connected) {
    ...
    p139_check('Canonical database tables', $missing ? 'FAIL' : 'PASS', $missing ? implode(', ', $missing) : count($expectedTables) . ' tables available');
    ...
    foreach ($requiredColumns as $table => $columns) {
        ...
        p139_check($table . ' columns', $missingColumns ? 'FAIL' : 'PASS', $missingColumns ? implode(', ', $missingColumns) : count($columns) . ' required columns');
    }

    $weeklySchema = weekly_test_schema_status();
    p139_check('Weekly Test schema', ($weeklySchema['ready'] ?? false) ? 'PASS' : 'FAIL', implode(', ', $weeklySchema['missing'] ?? []));
    foreach (['basic','previous','upcoming'] as $type) {
        try {
            ...
            p139_check(ucfirst($type) . ' test mapping', !$tests ? 'WARN' : ($questionCount > 0 ? 'PASS' : 'FAIL'), count($tests) . ' paper(s), ' . $ready . ' ready, ' . $questionCount . ' question(s)');
        } catch (Throwable $exception) {
            p139_check(ucfirst($type) . ' test mapping', 'FAIL', 'Query failed.');
        }
    }
    ...
    foreach ($moduleQueries as $label => $sql) {
        try {
            $count = (int)p139_scalar($sql);
            p139_check($label, $count > 0 ? 'PASS' : 'WARN', (string)$count);
        } catch (Throwable $exception) {
            p139_check($label, 'FAIL', 'Query failed.');
        }
    }

    foreach (['facebook_url','instagram_url','youtube_url','linkedin_url','twitter_url'] as $settingKey) {
        try {
            $value = trim((string)p139_scalar('SELECT setting_value FROM site_settings WHERE setting_key=? LIMIT 1', [$settingKey]));
            p139_check('Footer setting: ' . $settingKey, $value !== '' ? 'PASS' : 'WARN', $value !== '' ? 'Configured' : 'Blank; icon will remain hidden');
        } catch (Throwable $exception) {
            p139_check('Footer setting: ' . $settingKey, 'FAIL', 'Query failed.');
        }
    }
    ...
    try {
        ...
        p139_check('Core insert statements', 'PASS', 'Admission, student and weekly-attempt statements prepared.');
    } catch (Throwable $exception) {
        p139_check('Core insert statements', 'FAIL', 'One or more statements could not be prepared.');
    }

    if (filter_var(getenv('PHASE139_WRITE_TESTS') ?: 'false', FILTER_VALIDATE_BOOL)) {
        $pdo = db();
        try {
            ...
            $pdo->prepare('INSERT INTO enquiries (name, phone, course_interest, current_level, preferred_batch, lead_source, message, enquiry_status, ip_address, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())')
                ->execute(['Phase137 Check', $phone, 'Basic Spoken English', 'Basic', 'Morning', 'Phase137 CLI', 'Rollback test', 'New', '127.0.0.1']);
            $pdo->prepare('INSERT INTO students (full_name, phone, email, password_hash, current_level, target_goal, preferred_language, daily_goal_minutes, published, status_deleted, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NOW())')
                ->execute(['Phase137 Student', $phone, null, password_hash('Temporary#137', PASSWORD_DEFAULT), 'Basic', 'Verification', 'Hindi', 20, 'Yes']);
            $inserted = (int)p139_scalar('SELECT COUNT(*) FROM enquiries WHERE phone=?', [$phone]) === 1
                && (int)p139_scalar('SELECT COUNT(*) FROM students WHERE phone=?', [$phone]) === 1;
            $pdo->rollBack();
            p139_check('Transactional write/rollback', $inserted ? 'PASS' : 'FAIL', $inserted ? 'Rows inserted and rolled back.' : 'Inserted rows were not readable.');
        } catch (Throwable $exception) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            p139_check('Transactional write/rollback', 'FAIL', 'Write test failed; no data was committed.');
        }
    } else {
        p139_check('Transactional write/rollback', 'WARN', 'Skipped. Run with PHASE139_WRITE_TESTS=true on staging/local DB.');
    }
}

$storage = dirname(__DIR__) . '/storage';
p139_check('Storage directory', is_dir($storage) && is_writable($storage) ? 'PASS' : 'FAIL', $storage);
$rateLimits = $storage . '/rate-limits';
if (!is_dir($rateLimits)) @mkdir($rateLimits, 0775, true);
p139_check('Rate-limit directory', is_dir($rateLimits) && is_writable($rateLimits) ? 'PASS' : 'FAIL', $rateLimits);

$width = max(32, max(array_map(static fn(array $row): int => strlen($row[0]), $rows)) + 2);
echo "Phase 139 Functional Check\n" . str_repeat('=', 92) . "\n";
