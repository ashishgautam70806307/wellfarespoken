...
function p137_check(string $name, string $status, string $detail = ''): void
{
...
function p137_scalar(string $sql, array $params = []): mixed
{
...
p137_check('Runtime environment', 'PASS', APP_RUNTIME_ENV . ' / ' . APP_ENV);
p137_check('Database profile', 'PASS', DB_HOST . ':' . DB_PORT . '/' . DB_NAME . ' as ' . DB_USER);
p137_check('Runtime schema updates', APP_ALLOW_SCHEMA_UPDATES ? 'WARN' : 'PASS', APP_ALLOW_SCHEMA_UPDATES ? 'Enabled; disable after import.' : 'Disabled');
p137_check('Application debug', (APP_RUNTIME_ENV === 'live' && APP_DEBUG) ? 'FAIL' : 'PASS', APP_DEBUG ? 'Enabled' : 'Disabled');
p137_check('UTF-8 compatibility', function_exists('mb_strlen') && function_exists('mb_substr') && function_exists('mb_strimwidth') ? 'PASS' : 'FAIL', extension_loaded('mbstring') ? 'mbstring' : 'compatibility fallback');

$mysqlDriver = in_array('mysql', PDO::getAvailableDrivers(), true);
p137_check('PDO MySQL driver', $mysqlDriver ? 'PASS' : 'FAIL', implode(', ', PDO::getAvailableDrivers()) ?: 'No PDO drivers');
$connected = false;
if ($mysqlDriver) {
    try {
        db();
        $connected = true;
        p137_check('Database connection', 'PASS');
    } catch (Throwable $exception) {
        p137_check('Database connection', 'FAIL', 'Connection failed. Verify local/live .env settings.');
    }
} else {
    p137_check('Database connection', 'FAIL', 'pdo_mysql is not installed in this PHP runtime.');
}
...
if ($connected) {
    ...
    p137_check('Canonical database tables', $missing ? 'FAIL' : 'PASS', $missing ? implode(', ', $missing) : count($expectedTables) . ' tables available');
    ...
    foreach ($requiredColumns as $table => $columns) {
        ...
        p137_check($table . ' columns', $missingColumns ? 'FAIL' : 'PASS', $missingColumns ? implode(', ', $missingColumns) : count($columns) . ' required columns');
    }

    $weeklySchema = weekly_test_schema_status();
    p137_check('Weekly Test schema', ($weeklySchema['ready'] ?? false) ? 'PASS' : 'FAIL', implode(', ', $weeklySchema['missing'] ?? []));
    foreach (['basic','previous','upcoming'] as $type) {
        try {
            ...
            p137_check(ucfirst($type) . ' test mapping', !$tests ? 'WARN' : ($questionCount > 0 ? 'PASS' : 'FAIL'), count($tests) . ' paper(s), ' . $ready . ' ready, ' . $questionCount . ' question(s)');
        } catch (Throwable $exception) {
            p137_check(ucfirst($type) . ' test mapping', 'FAIL', 'Query failed.');
        }
    }
    ...
    foreach ($moduleQueries as $label => $sql) {
        try {
            $count = (int)p137_scalar($sql);
            p137_check($label, $count > 0 ? 'PASS' : 'WARN', (string)$count);
        } catch (Throwable $exception) {
            p137_check($label, 'FAIL', 'Query failed.');
        }
    }

    foreach (['facebook_url','instagram_url','youtube_url','linkedin_url'] as $settingKey) {
        try {
            $value = trim((string)p137_scalar('SELECT setting_value FROM site_settings WHERE setting_key=? LIMIT 1', [$settingKey]));
            p137_check('Footer setting: ' . $settingKey, $value !== '' ? 'PASS' : 'WARN', $value !== '' ? 'Configured' : 'Blank; icon will remain hidden');
        } catch (Throwable $exception) {
            p137_check('Footer setting: ' . $settingKey, 'FAIL', 'Query failed.');
        }
    }
    ...
    try {
        ...
        p137_check('Core insert statements', 'PASS', 'Admission, student and weekly-attempt statements prepared.');
    } catch (Throwable $exception) {
        p137_check('Core insert statements', 'FAIL', 'One or more statements could not be prepared.');
    }

    if (filter_var(getenv('PHASE137_WRITE_TESTS') ?: 'false', FILTER_VALIDATE_BOOL)) {
        $pdo = db();
        try {
            ...
            $inserted = (int)p137_scalar('SELECT COUNT(*) FROM enquiries WHERE phone=?', [$phone]) === 1
                && (int)p137_scalar('SELECT COUNT(*) FROM students WHERE phone=?', [$phone]) === 1;
            $pdo->rollBack();
            p137_check('Transactional write/rollback', $inserted ? 'PASS' : 'FAIL', $inserted ? 'Rows inserted and rolled back.' : 'Inserted rows were not readable.');
        } catch (Throwable $exception) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            p137_check('Transactional write/rollback', 'FAIL', 'Write test failed; no data was committed.');
        }
    } else {
        p137_check('Transactional write/rollback', 'WARN', 'Skipped. Run with PHASE137_WRITE_TESTS=true on staging/local DB.');
    }
}

$storage = dirname(__DIR__) . '/storage';
p137_check('Storage directory', is_dir($storage) && is_writable($storage) ? 'PASS' : 'FAIL', $storage);
$rateLimits = $storage . '/rate-limits';
if (!is_dir($rateLimits)) @mkdir($rateLimits, 0775, true);
p137_check('Rate-limit directory', is_dir($rateLimits) && is_writable($rateLimits) ? 'PASS' : 'FAIL', $rateLimits);

$width = max(32, max(array_map(static fn(array $row): int => strlen($row[0]), $rows)) + 2);
echo "Phase 137 Functional Check\n" . str_repeat('=', 92) . "\n";
