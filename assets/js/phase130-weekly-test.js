(function () {
  ...
  function openRequestedType() {
    const params = new URLSearchParams(window.location.search);
    const requested = String(params.get('type') || '').toLowerCase();
    if (!['basic', 'previous', 'upcoming'].includes(requested)) return;
    const trigger = document.querySelector('[data-select-test="' + requested + '"]');
    if (trigger && !trigger.disabled) {
      window.setTimeout(() => openSetup(requested, trigger), 120);
    }
  }
  ...
  startButton?.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('weekly-test-api.php', {
        method: 'POST',
        body,
      ...
      if (!contentType.includes('application/json')) {
        throw new Error(response.ok ? 'Unexpected server response. Refresh the page and try again.' : 'Server error. Please try again.');
      }
