...
if (!empty($_GET['delete']) && csrf_validate($_GET['token'] ?? '')) {
    db()->prepare('DELETE FROM form_options WHERE id = ?')->execute([(int)$_GET['delete']]);
...
<div class="table-card"><table class="admin-table"><thead><tr><th>Order</th><th>Label</th><th>Value</th><th>Published</th><th>Actions</th></tr></thead><tbody><?php foreach ($rows as $row): ?><tr><td><?= e((string)$row['sort_order']) ?></td><td><strong><?= e($row['option_label']) ?></strong><br><small><?= e($row['helper_text'] ?: 'No helper text') ?></small></td><td><?= e($row['option_value'] ?: $row['option_label']) ?></td><td><span class="badge <?= $row['published'] === 'Yes' ? 'badge-green' : 'badge-gray' ?>"><?= e($row['published']) ?></span></td><td><a class="btn btn-sm btn-soft" href="form-options.php?group=<?= e($group) ?>&edit=<?= e((string)$row['id']) ?>">Edit</a> <a class="btn btn-sm btn-danger" onclick="return confirm('Delete this option?')" href="form-options.php?group=<?= e($group) ?>&delete=<?= e((string)$row['id']) ?>&token=<?= e(csrf_token()) ?>">Delete</a></td></tr><?php endforeach; if (!$rows): ?><tr><td colspan="5">No options found.</td></tr><?php endif; ?></tbody></table></div>
