...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && csrf_validate($_POST['csrf_token'] ?? '')) {
    $action = $_POST['action'] ?? '';
    if ($action === 'update') {
        $stmt = db()->prepare('UPDATE enquiries SET enquiry_status=?, lead_priority=?, follow_up_date=?, last_contacted_at=?, admin_note=? WHERE id=?');
    ...
    if ($action === 'delete') {
        ...
        flash('success', 'Enquiry deleted.'); redirect('enquiries.php');
    }
}
$q = trim($_GET['q'] ?? '');
$status = trim($_GET['status'] ?? '');
...
if ($q !== '') { $where[]='(name LIKE ? OR phone LIKE ? OR course_interest LIKE ? OR message LIKE ?)'; $params[]="%$q%";$params[]="%$q%";$params[]="%$q%";$params[]="%$q%"; }
if ($status !== '') { $where[]='enquiry_status=?'; $params[]=$status; }
$sql='SELECT * FROM enquiries'.($where?' WHERE '.implode(' AND ',$where):'').' ORDER BY id DESC LIMIT 300';
...
if (isset($_GET['edit'])) { $s=db()->prepare('SELECT * FROM enquiries WHERE id=?');$s->execute([(int)$_GET['edit']]);$edit=$s->fetch(); }
