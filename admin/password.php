...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try{
        ...
        if($action==='mfa_enable'){
            ...
            db()->prepare("UPDATE admins SET mfa_secret=?,mfa_enabled='Yes',auth_version=auth_version+1 WHERE id=?")->execute([$secret,$adminId]);unset($_SESSION['admin_mfa_setup_secret']);$fresh=db()->prepare('SELECT * FROM admins WHERE id=?');$fresh->execute([$adminId]);admin_session_login($fresh->fetch());admin_audit_log('admin.mfa_enabled','admin',$adminId,'TOTP MFA enabled.');flash('success','Authenticator MFA is now enabled.');redirect('password.php#mfa');
        }
        if($action==='mfa_disable'){
            $code=(string)($_POST['mfa_code']??'');if(($admin['mfa_enabled']??'No')!=='Yes'||empty($admin['mfa_secret'])||!admin_mfa_verify((string)$admin['mfa_secret'],$code))throw new RuntimeException('Enter a valid current authenticator code to disable MFA.');
    ...
    }catch(Throwable $e){flash('error',$e->getMessage());redirect('password.php'.(str_starts_with($action,'mfa_')?'#mfa':''));}
}
