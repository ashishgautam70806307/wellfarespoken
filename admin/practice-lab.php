...
if (isset($_GET['delete'], $_GET['table']) && csrf_validate($_GET['token'] ?? '')) {
    $table = safe_admin_table($_GET['table']);
    $id = (int)$_GET['delete'];
    if ($table && in_array($table, ['practice_categories','practice_lessons','practice_questions','practice_common_mistakes'], true)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && csrf_validate($_POST['csrf_token'] ?? '')) {
    ...
    if ($action === 'save_settings') {
        foreach (['practice_enabled','local_mode_enabled','browser_voice_enabled','ai_enabled','ai_correction_enabled','ai_fallback_enabled','ai_provider','openai_model','openai_endpoint','ai_daily_limit','ai_timeout_seconds','ai_temperature','ai_system_prompt','free_daily_limit','practice_intro_note'] as $key) {
            save_practice_setting($key, trim($_POST[$key] ?? ''));
        }
        $newKey = trim($_POST['openai_api_key'] ?? '');
        if ($newKey !== '') {
            save_practice_setting('openai_api_key', $newKey);
        }
...
<div class="grid-2">
    <div class="panel-card">
        ...
        <form method="post" class="form-grid">
            ...
            <label class="full">OpenAI API Key <small>Leave blank to keep existing key.</small><input type="password" name="openai_api_key" placeholder="sk-..."></label>
            ...
            <label class="full">API Endpoint<input name="openai_endpoint" value="<?= e(practice_setting('openai_endpoint','https://api.openai.com/v1/chat/completions')) ?>"></label>
    ...
    <div class="panel-card">
        ...
        <div class="visual-list admin-checklist">
            ...
            <div><span>API key is optional</span><b>Admin controlled</b></div>
        ...
        <p class="muted-text">For your institute website, this is the correct advanced setup: local practice first, optional AI second. Visitors always get feedback; AI only improves the feedback when available.</p><p class="muted-text"><strong>Important:</strong> The feature is free in local mode. Real AI correction needs an API key, but the visitor experience will not break if the key is empty, API is off, or limit is reached.</p>
    </div>
</div>
<?php else: ?>
