...
if ($remaining <= 0) {
    db()->prepare("UPDATE weekly_test_attempts SET status='submitted', submitted_at=NOW(), submission_reason='timer_expired' WHERE id=? AND access_token=? AND status='started'")
        ->execute([$attemptId, $token]);
    header('Location: ' . $resultUrl);
    exit;
}

$safe = weekly_test_snapshot_questions($attempt, false);
if (!$safe) {
    ...
    $questions = array_slice($questions, 0, (int)($attempt['total_questions'] ?: 30));
    ...
    db()->prepare("UPDATE weekly_test_attempts SET question_snapshot=? WHERE id=? AND access_token=?")
        ->execute([json_encode($snapshot, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), $attemptId, $token]);
